...
static constexpr unsigned int _Table[256] = 
{
...
class HashModelCRC32 : public IHashModel
{
public:
    ...
    virtual void update(const void *data, size_t len) override final
    {
        ...
        {
            const uint8_t inputByte = ((const uint8_t*)data)[i];
            hash = _Table[(hash ^ inputByte) & 0xFF] ^ (hash >> 8);
        }
        hash ^= 0xFFFFFFFF;
    }
...
{
    return std::make_unique<HashModelCRC32>();
}
